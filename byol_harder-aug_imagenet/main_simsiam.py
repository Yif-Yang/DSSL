...
parser.add_argument('--work_dir',  type=str, default='./byol_trans2/',
                    help='name of data list file')
...
parser.add_argument('--hard_N', type=int, default=4)
parser.add_argument('--hard_M', type=int, default=9)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_transforms = SimSiamTransform(224, mean_std=imagenet_mean_std, N=args.N, M=args.M, hard_N=args.hard_N, hard_M=args.hard_M, grid=args.grid)
    train_dataset = Imagenet(
        traindir, args.train_list, train=True,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=os.path.join(saved_path, 'checkpoint_{:04d}.pth.tar'.format(epoch)))
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        loss += model.module.forward_single(image_one=images[2], image_two=images[0])
        loss += model.module.forward_single(image_one=images[3], image_two=images[1])
