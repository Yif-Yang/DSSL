...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            for old_key, value in state_dict.items():
                if old_key.startswith('module.backbone') and 'fc' not in old_key:
    ...
    test_dataset = Imagenet(
                traindir,  args.test_list, train=False,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        now_acc = {'acc':acc1, 'acc5':acc5}
        gather_metrics(now_acc)
        mean_acc = torch.mean(now_acc['acc'])
        mean_acc_5 = torch.mean(now_acc['acc5'])
        logger.info(f'in gpu:{gpu}, now_acc is {now_acc}, while mean_acc1 is {mean_acc}, mean_acc5 is {mean_acc_5}')
        acc1 = mean_acc
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, filename=saved_path+'linear_checkpoint_{:04d}.pth.tar'.format(epoch))
